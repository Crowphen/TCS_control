...
class TcsDevice:
    ...
    def set_durations(self, stimDurations):
        ...
        if all(x == stimDurations[0] for x in stimDurations):
    ...
    def set_ramp_speed(self, rampSpeeds):
        ...
        # check if speeds are equal
        if all(x == rampSpeeds[0] for x in rampSpeeds):
            # yes: send all speeds in one command
    ...
    def set_return_speed(self, returnSpeeds):
        ...
        # check if speeds are equal
        if all(x == returnSpeeds[0] for x in returnSpeeds):
            # yes: send all speeds in one command
    ...
    def set_temperatures(self, temperatures):
        ...
        :param temperatures: array of 5 values (min 0.1°C, max 51°C)
        """
        for i in range(5):
            if temperatures[i] > 51:
                temperatures[i] = 51
